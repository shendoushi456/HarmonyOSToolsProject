...
class _DocumentCapturePreviewPageState
  ...
  Future<void> _watermark() async {
    final text = await showDialog<String>(
      context: context,
      builder: (dialogContext) => const WatermarkDialog(),
    );
...
/// 水印文字输入对话框。
///
/// TextEditingController 生命周期绑定到本 State：对话框 route 退出动画结束、
/// TextField 从组件树移除后才触发 dispose，避免
/// "A TextEditingController was used after being disposed" 断言导致红屏。
...
class _WatermarkDialogState extends State<WatermarkDialog> {
  final TextEditingController _controller =
      TextEditingController(text: '默认水印');
