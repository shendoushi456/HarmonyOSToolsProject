...
// 结构：日历页同款渐变背景 → 顶栏(城市/日期/农历+农业天气) → 农业天气卡
...
import '../../weather/viewmodels/qx_home_view_model.dart';
...
class NongyePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // 城市跟随天气页(QxHomePage)顶部选择的城市：监听 QxHome 的 cityName 联动加载
    ref.listen<String>(
      qxHomeViewModelProvider.select((item) => item.cityName),
      (previous, city) {
        if (city.isNotEmpty && city != previous) {
    ...
    return Scaffold(
      ...
      body: LayoutBuilder(builder: (context, constraints) {
        // 背景改用日历页(QxCalendarPage)同款渐变：
        // Compose verticalGradient endY=760px（xxhdpi 3x）≈ 253 逻辑像素处渐变结束
        final whiteStop =
            (253.0 / constraints.maxHeight).clamp(0.0, 1.0);
...
class _AgricultureTopBar extends StatelessWidget {
  ...
  Future<void> _selectCity(BuildContext context, WidgetRef ref) async {
    ...
    if (changed == true) {
      // 刷新天气页城市（同一 CityRepository 数据源），
      // QxHome 的 cityName 变化会经 listen 联动加载新城市农业数据
